...
export default function MainContent({ onFirstPromptSent, activeChatId, messages, setMessages }) {
  ...
  const handleSendMessage = async () => {
    ...
    const newUserMessage = { id: Date.now(), text: currentPrompt, sender: "user" };
    setMessages((prevMessages) => [...prevMessages, newUserMessage]); // Update messages state
    setInput("");
    setIsTyping(true);

    // If this is the very first message of a new session, notify the parent to add to sidebar
    if (isFirstMessage) {
      onFirstPromptSent(currentPrompt); // Pass the actual prompt text
    }
    ...
    try {
      ...
      setMessages((prevMessages) => [...prevMessages, aiResponse]); // Update messages state
    } catch (error) {
      ...
      setMessages((prevMessages) => [...prevMessages, errorMessage]); // Update messages state
    } finally {
      setIsTyping(false);
    }
  };
      ...
      <div className="flex-1 overflow-y-auto pt-16 pb-24 px-4 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-950">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center px-8">
            ...
            <h2 className="text-3xl font-bold mb-4 text-gray-200">Hello there! How can I help you today?</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-2xl mt-8">
              <button
                className="bg-gray-800 hover:bg-gray-700 p-4 rounded-lg text-left transition-colors flex items-center gap-3"
                onClick={() => setInput("Suggest a dish for dinner with ingredients I have.")}
              >
              ...
              <button
                className="bg-gray-800 hover:bg-gray-700 p-4 rounded-lg text-left transition-colors flex items-center gap-3"
                onClick={() => setInput("Write a short story about a space explorer.")}
              >
        ...
        <div className="relative flex items-center max-w-3xl mx-auto bg-gray-800 rounded-full pr-2">
          ...
          <button
            ...
            className={`p-2 ml-2 rounded-full ${
              input.trim() ? "bg-blue-600 hover:bg-blue-700" : "bg-gray-700 cursor-not-allowed"
            } transition-colors`}
            disabled={!input.trim()}
          >
